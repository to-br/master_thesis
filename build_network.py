...
def build_network(sim_time=1000, second_sine=False, noisy=True,
                  f1=10.0, a=100.0, SD=250.0, delay_mean=1, delay_sd=0.25,
                  V_thresh=-50.0, E_m=-60.0, tau_m=10, weight=1,
                  seed=np.random.randint(0,1e6), beat=20, resolution=0.25,
                  measure_from_A=False, num_A=1000):
    ...
    nodes_A = nest.Create("iaf_psc_alpha", num_A, params=params)
    node_B = nest.Create("iaf_psc_alpha", 1, params=params)
  ...
  # Connect B cell to population A
    if delay_sd != 0:
        ...
        for idx, node in enumerate(nodes_A):
            nest.Connect(node, node_B, syn_spec={"weight": weight, "delay": random_delays[idx]})
    
    else:
        nest.Connect(nodes_A, node_B, syn_spec={"weight": weight, "delay" : delay_mean})
    ...
    if measure_from_A == False:
        ...
        events = spike_recorders[2].get("events") 
